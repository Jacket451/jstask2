...
const forms = () => {
  ...
  form.forEach(item => {
    item.addEventListener('submit', e => {
      ...
      console.log(api);
      Object(_services_request__WEBPACK_IMPORTED_MODULE_1__["postData"])(api, formData).then(res => {
        console.log(res);
  ...
  links.forEach(link => {
    link.addEventListener('click', function (event) {
      ...
      let widthTop = document.documentElement.scrollTop,
          hash = this.hash,
          toBlock = document.querySelector(hash).hetBoundingClientRect().top,
          start = null;
...
const showMoreStyles = (trigger, wrapper) => {
  ...
  btn.addEventListener('click', function () {
    Object(_services_request__WEBPACK_IMPORTED_MODULE_0__["getResource"])('http://localhost:3000/styles').then(res => createCards(res)).catch(error => errorCards(error));
    this.remove();
  });
  ...
  function errorCards() {
    ...
    card.innerHTML = `
                <div class="styles-block">
                    ...
                    <h4>Проблемы с сервером. Просим прощение за неудобства</h4>
                </div>
            `;
    document.querySelector(wrapper).appendChild(card);
  }
